 ...
 * scheduled, and would have failed on every run until someone noticed: no venue swaps a token for
 * itself (on the Base build 1inch answered "src and dst should be different"; on X Layer there is no
 * USDC→USDC pool for Uniswap v3 to route through).
 *
 ...
 * spelled `NVDAx` and an uppercase compare here would be a second, subtly different definition of
 * "the same symbol" — which is how two checks drift apart until one of them means nothing.
 */
import { describe, expect, it } from 'vitest';

// ESM hoists imports above assignments, so the registry is pulled in dynamically, after the chain it reads is set.
process.env.XORR_CHAIN ??= 'xlayer-testnet';
const { SETTLEMENT_SYMBOL, TOKENS, canonicalSymbol } = await import('../venues/tokens.js');
...
describe('the settlement token is not a buy target', () => {
  ...
  it('still allows everything a buy can actually route into', () => {
    for (const symbol of ['WOKB', 'XBTC', 'USDG', 'NVDAx', 'AAPLx']) {
