...
|---|---|---|---|---|
| 1 | 0:00–0:20 | **The janitor film** (`~/Desktop/xorr-night-shift.mp4`) — market closed, 3:02 a.m., the phone lights up, a trade is done | *(the film's own sound — no voice)* | done |
...
| 5 | 1:05–1:15 | **iPhone: test funds** — Deposit → "Get 1,000 test USDC" → the balance lands | "On the demo network, test funds arrive in seconds." | **you** |
...
| 7 | 1:45–2:35 | **iPhone: create your agent** — the + on Home's agents → Name → What it does → Works like (pick **Momentum Scout**) → Pick strategies → choose one or two (each shows 90 days of real results) → Daily limit → **Make agent** → its page opens | "Now I make my own agent. I name it, tell it what it's for, choose how it behaves, and give it strategies — each one shows what it did on real prices. It gets its own spending limit on top of the permission." | **you** |
| 8 | 2:35–3:05 | **The agent trades on its own** — Activity: Momentum Scout bought METAx, routed through **OKX DEX** (it did, unprompted, on 2026-09-25 08:30 UTC — the demo account `test-3570`) → tap → why it bought, and the transaction | "It traded while I wasn't looking — routed through OKX DEX because OKX paid more than Uniswap — and it tells me why." | Claude |
| 9 | 3:05–3:20 | **It can't overspend** — a buy past today's cap is refused in words; the contract would refuse it anyway | "Past the cap, it's refused — in the app, and on chain." | Claude |
| 10 | 3:20–3:32 | **Stop all** — hold the button → "Trading stopped · confirmed on-chain" | "And one hold takes it all back. Signed by me. No server in the way." | Claude |
| 11 | 3:32–3:42 | **Check it yourself** — `/judge`: 20 of 20 claims re-checked live against the chain | "Every claim the app makes, checked against the chain — live, by anyone." | Claude |
| 12 | 3:42–3:45 | **End card** — XORR. · xorr-xlayer.vercel.app · github.com/nickthelegend/xorr-xlayer | *(music out)* | Claude |

---

## Your part (shots 4–7): how to record it
...
Do shots 4 → 7 in one go (mistakes are fine — pause a second and redo the step; it gets cut). Press **Ctrl-C** in
...
## What Claude records (shots 2, 3, 8–12)

On the web app, signed in as the demo account that already has a permission, funds and an agent that has traded —
recorded with `tools/record-demo.mjs` at phone size, so it cuts cleanly with the iPhone footage.
...
|---|---|
| Project | xorr |
| Track | X Layer — tokenized stocks & RWAs |
