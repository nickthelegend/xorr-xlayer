...
- **P4.9 Privy on X Layer** — `BLOCKED` on owner (Privy dashboard: X Layer chains, `xorr-xlayer.vercel.app` origin). The hosted `/verify` Privy-policy check answers Privy 500 until then. on owner dashboard (S). Enable chains 196/1952 for embedded wallets; enable Google/X login (code shipped in xorr-eth `f7be1b3`, present here); verify grant/revoke signing on the fork via `walletSignsOnly` (`src/wallet/userSigning.ts`).
- **P4.10 OKX Wallet sign-in on web** — `DONE` in code (`efea4ff`): OKX Wallet first in Privy's web wallet list (`okx_wallet`, type-checked against @privy-io/react-auth 3.40). Live connect needs the Privy dashboard origin (P4.9).
...
- **P5.4 `/judge` green on the deployed executor** — `DONE` except Privy (P4.9): every non-Privy check passes or skips with its reason on the hosted executor; the web `/judge` renders it live.
