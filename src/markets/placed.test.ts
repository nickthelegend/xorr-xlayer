...
describe('a first attempt', () => {
  it('says what was bought', () => {
    expect(placementOf({ side: 'buy', symbol: 'WETH', units: 0.0412, replayed: false })).toEqual({
      label: 'Bought 0.0412 WETH',
      replayed: false,
    });
  });

  it('says what was sold', () => {
    expect(placementOf({ side: 'sell', symbol: 'CBBTC', units: 0.0031, replayed: false }).label).toBe(
      'Sold 0.0031 CBBTC',
    );
  });

  it('has nothing extra to explain', () => {
    expect(placementOf({ side: 'buy', symbol: 'WETH', units: 1, replayed: false }).note).toBeUndefined();
  });
});

describe('the same attempt again', () => {
  it('says already, and says nothing was placed a second time', () => {
    const p = placementOf({ side: 'buy', symbol: 'WETH', units: 0.0412, replayed: true });
    expect(p.label).toBe('Already bought 0.0412 WETH');
  ...
  it('never reads as a second fill', () => {
    ...
    const p = placementOf({ side: 'buy', symbol: 'WETH', units: 0.0412, replayed: true });
    expect(p.label).not.toBe('Bought 0.0412 WETH');
...
describe('a quantity that was not reported', () => {
  it('names the token rather than quoting a zero', () => {
    // A watch-mode or skipped run reports no units, and "Bought 0.0000 WETH" would be a false number.
    expect(placementOf({ side: 'buy', symbol: 'WETH', units: undefined, replayed: false }).label).toBe('Bought WETH');
    expect(placementOf({ side: 'buy', symbol: 'WETH', units: 0, replayed: true }).label).toBe('Already bought WETH');
  });
});
